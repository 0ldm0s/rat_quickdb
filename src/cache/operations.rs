...
use crate::types::{CacheConfig, DataValue, IdType};
...
impl CacheManager {
    ...
    pub async fn clear_by_pattern(&self, pattern: &str) -> Result<usize> {
        ...
        for (table_name, keys) in table_keys.iter() {
            ...
            for key in keys {
                if self.matches_pattern(key, pattern) {
                    if let Err(e) = self.cache.delete(key).await {
                        warn!(
                            "删除匹配模式的缓存键失败: key={}, pattern={}, error={}",
                            key, pattern, e
                        );
                    } else {
                        ...
                        cleared_count += 1;
                        info!(
                            "已删除匹配模式的缓存键: table={}, key={}, pattern={}",
                            table_name, key, pattern
                        );
                    }
                }
            }
        ...
        debug!(
            "按模式清理缓存完成: pattern={}, cleared_count={}",
            pattern, cleared_count
        );
        Ok(cleared_count)
    }
    ...
    pub async fn clear_records_batch(&self, table: &str, ids: &[IdType]) -> Result<usize> {
        ...
        info!(
            "批量清理记录缓存完成: table={}, total={}, cleared={}",
            table,
            ids.len(),
            cleared_count
        );
    ...
    /// * `table` - 表名
    /// * `records` - 记录列表
    pub async fn cache_records_batch_optimized(
        &self,
        table: &str,
    ...
    /// * `table` - 表名
    /// * `records` - 记录列表
    pub async fn cache_records_batch(
        &self,
        table: &str,
        records: Vec<(IdType, DataValue)>,
    ) -> Result<usize> {
        if !self.config.enabled {
            return Ok(0);
        }
