    ...
    for (field_name, field_def) in fields {
        if let Some(current_value) = data_map.get::<str>(field_name) {
            let converted_value = match current_value {
                ...
                DataValue::String(s) if matches!(field_def.field_type, crate::model::FieldType::DateTimeWithTz { .. }) => {
                    match chrono::DateTime::parse_from_rfc3339(s) {
                        Ok(dt) => {
                            rat_logger::debug!("字段 {} String转DateTime: {} -> {}", field_name, s, dt);
                            Some(DataValue::DateTime(dt.with_timezone(&chrono::FixedOffset::east(0))))
                        },
                        Err(e) => {
                            rat_logger::debug!("字段 {} String转DateTime失败: {} (错误: {})", field_name, s, e);
                            None
                        }
                    }
                },
                // 处理DateTimeWithTz字段的DateTimeUTC类型转换
                DataValue::DateTimeUTC(dt) if matches!(field_def.field_type, crate::model::FieldType::DateTimeWithTz { .. }) => {
