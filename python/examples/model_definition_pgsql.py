...
# -*- coding: utf-8 -*-
"""
RatQuickDB Python模型定义示例（PostgreSQL版本）

本示例展示了如何使用RatQuickDB的应用模式进行模型定义，
包括字段定义、索引创建、模型验证等功能，对应主库model_definition_pgsql.rs示例。
"""
...
class Article:
    ...
    # 关联字段
    author_id = rq.uuid_field(
        required=True,
        unique=False,
        description="作者ID"
    )

    category_id = rq.uuid_field(
        required=False,
        unique=False,
        description="分类ID"
    )
...
class Comment:
    ...
    article_id = rq.uuid_field(
        required=True,
        unique=False,
        description="文章ID"
    )

    user_id = rq.uuid_field(
        required=True,
        unique=False,
        description="用户ID"
    )

    parent_id = rq.uuid_field(
        required=False,
        unique=False,
        description="父评论ID"
    )
...
def demonstrate_json_serialization():
    ...
    try:
        ...
        if insert_result.get("success"):
            ...
            print("\n=== 查询所有用户 ===")
            users = User.find()
            print(f"找到用户数量: {len(users.get('data', []))}")
            for user in users.get('data', []):
                print(f"用户: {user['name']}, 邮箱: {user['email']}, 状态: {user['status']}")
...
def main():
    """主函数"""
    print("RAT QuickDB Python绑定 - 模型定义示例（PostgreSQL版本）")
    print("=" * 60)
    ...
    try:
        ...
        # 添加PostgreSQL数据库到应用
        result = app.add_postgresql_database(
            alias="default",
            host="172.16.0.23",
            port=5432,
            database="testdb",
...
if __name__ == "__main__":
    success = main()
    if success:
        print("\n✅ Python模型定义示例（PostgreSQL版本）演示完成！")
        sys.exit(0)
    else:
        print("\n❌ Python模型定义示例（PostgreSQL版本）演示失败！")
        sys.exit(1)