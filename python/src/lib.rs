...
/// 简单测试函数 - 用于验证PyO3绑定是否正常工作
#[pyfunction]
fn test_function() -> String {
    "PyO3绑定正常工作".to_string()
}

/// 初始化全局任务队列系统
///
/// 这个函数必须在执行任何数据库操作之前调用，用于启动工作线程系统
///
/// 参数:
/// - worker_count: 工作线程数量，默认为CPU核心数
///
/// 注意：这个函数只需要调用一次，多次调用会返回错误
/// 注意：根据v0.3.0+架构，Python集成应该通过JSON队列桥接器发送指令，
/// 而不是直接初始化任务队列。这个函数暂时保留但标记为已弃用。
#[pyfunction]
fn init_task_queue(worker_count: Option<usize>) -> PyResult<()> {
    // 根据v0.3.0+架构，Python集成应该通过JSON队列桥接器发送指令
    // 任务队列会在第一次使用时自动初始化
    // 这个函数暂时保留但不再执行实际初始化
    Ok(())
}
...
/// 关闭全局任务队列系统
///
/// 这个函数应该在应用程序退出时调用，用于优雅地关闭工作线程
/// 注意：根据v0.3.0+架构，Python集成应该通过JSON队列桥接器发送指令，
/// 而不是直接管理任务队列。这个函数暂时保留但标记为已弃用。
#[pyfunction]
fn shutdown_task_queue() -> PyResult<()> {
    // 根据v0.3.0+架构，Python集成应该通过JSON队列桥接器发送指令
    // 任务队列的生命周期由系统自动管理
    // 这个函数暂时保留但不再执行实际关闭操作
    Ok(())
...
fn rat_quickdb_py(_py: Python, m: &PyModule) -> PyResult<()> {
    ...
    // 任务队列初始化函数
    m.add_function(wrap_pyfunction!(init_task_queue, m)?)?;
    m.add_function(wrap_pyfunction!(shutdown_task_queue, m)?)?;
    ...
    // JSON队列桥接器
    m.add_class::<rat_quickdb::python_api::json_queue_bridge::PyJsonQueueBridge>()?;
    m.add_function(wrap_pyfunction!(create_json_queue_bridge, m)?)?;
