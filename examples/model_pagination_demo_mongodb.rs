...
async fn main() -> QuickDbResult<()> {
    ...
    // 设置默认数据库别名
    rat_quickdb::set_default_alias("mongodb_main").await?;

    // 清理旧集合
    println!("清理旧集合...");
    let _ = rat_quickdb::drop_table("mongodb_main", "employees").await;
    println!("✅ 集合清理完成");
    ...
    if let Some(&is_healthy) = health_results.get("mongodb_main") {
        if is_healthy {
            println!("✅ 数据库连接正常");
        } else {
            println!("❌ 数据库连接异常");
        ...
        println!("❌ 未找到mongodb_main数据库配置");
        return Err(QuickDbError::ConnectionError {
            message: "未找到mongodb_main数据库配置".to_string(),
        });
    }
    println!();
    ...
    // 先清空现有数据
    let _ = rat_quickdb::delete("employees", vec![], Some("mongodb_main")).await;
    ...
    match rat_quickdb::delete("employees", vec![], Some("mongodb_main")).await {
