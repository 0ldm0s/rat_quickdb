...
impl CachePerformanceTest {
    ...
    async fn setup_test_data(&mut self) -> QuickDbResult<()> {
        ...
        let non_cached_test_users = vec![
            self.create_user("user1", "zhangsan_noncached@example.com", 25, "北京"),
            self.create_user("user2", "lisi_noncached@example.com", 30, "上海"),
            self.create_user("user3", "wangwu_noncached@example.com", 28, "广州"),
            self.create_user("user4", "zhaoliu_noncached@example.com", 35, "深圳"),
            self.create_user("user5", "qianqi_noncached@example.com", 22, "杭州"),
        ];

        let non_cached_batch_users: Vec<User> = (6..=25)
            .map(|i| self.create_user(
                &format!("batch_user_{}", i),
    ...
    /// 创建用户数据
    fn create_user(&self, name: &str, email: &str, age: i32, city: &str) -> User {
        User {
    ...
    async fn warmup_cache(&mut self) -> QuickDbResult<()> {
        ...
        // 按ID查询预热
        let users = ModelManager::<User>::find(vec![], None).await?;
        if let Some(first_user) = users.first() {
            let _result = ModelManager::<User>::find_by_id(&first_user.id).await?;
        }
