...
async fn demonstrate_json_field_types() -> QuickDbResult<()> {
            ...
            match ModelManager::<User>::find_by_id(&created_id).await {
                Ok(Some(retrieved_user)) => {
                    ...
                    match ModelManager::<User>::find(json_query_conditions, None).await {
                        Ok(users) => {
                            assert!(users.len() > 0, "JSON字段查询（包含'Rust'技能）应该返回至少1个用户，但返回了0个用户");
    ...
    let article_with_metadata = Article {
        ...
        category_id: Some(uuid::Uuid::new_v4().to_string()),
...
async fn demonstrate_transaction_operations() -> QuickDbResult<()> {
    ...
    match ModelManager::<Article>::find(article_conditions, None).await {
        Ok(articles) => {
            assert!(articles.len() > 0, "事务操作中的关联文章查询应该返回至少1篇文章，但返回了0篇文章");
...
async fn demonstrate_model_validation() -> QuickDbResult<()> {
    ...
    // 使用ModelManager按用户名查询 - 使用实际创建的用户名
    if let Some(ref user_id) = created_user_id {
        match ModelManager::<User>::find_by_id(user_id).await {
            Ok(Some(first_user)) => {
                let username_conditions = vec![
                    QueryCondition {
                        field: "username".to_string(),
                        operator: QueryOperator::Eq,
                        value: DataValue::String(first_user.username.clone()),
                    }
                ];
                ...
                match ModelManager::<User>::find(username_conditions, None).await {
                    Ok(users) => {
                        ...
                        println!("✅ 按用户名查询结果: {} 个用户 (查询用户名: {})", users.len(), first_user.username);
                        for user in users {
                            println!("   用户: {} - {}", user.id, user.username);
                        }
                    },
            ...
            Ok(None) => println!("❌ 无法获取第一个用户信息进行查询测试"),
            Err(e) => println!("❌ 获取第一个用户失败: {}", e),
        }
    } else {
        println!("❌ 没有创建用户，跳过用户名查询测试");
    }
    ...
    // 只有当用户成功创建时，才创建关联文章
    let article_result = if let Some(ref user_id) = created_user_id {
        println!("✅ 使用用户ID {} 创建关联文章", user_id);
        ...
        println!("❌ 用户未创建，跳过文章创建");
        None
    };

    // 验证和保存文章数据
    if let Some(article) = article_result {
        // 验证文章数据
    ...
    if let Some(ref user_id) = created_user_id {
        match ModelManager::<User>::find_by_id(user_id).await {
            ...
            Ok(None) => println!("❌ 无法获取第一个用户信息进行存在性检查"),
            Err(e) => println!("❌ 获取第一个用户失败: {}", e),
        }
    } else {
        println!("❌ 没有创建用户，跳过存在性检查");
    }
...
async fn demonstrate_complex_queries() -> QuickDbResult<()> {
    ...
    match ModelManager::<User>::count(vec![]).await {
        Ok(total) => {
            assert!(total > 0, "复杂查询中的总用户数统计应该大于0，但返回了{}", total);
            println!("✅ 总用户数: {}", total);
        },
        Err(e) => println!("❌ 统计查询失败: {}", e),
    }

    // 5. 存在性查询（检查是否存在用户名包含'alice'的用户）
    println!("\n5. 存在性查询（检查是否存在用户名包含'alice'的用户）...");
    let exists_conditions = vec![
        QueryCondition {
            field: "username".to_string(),
