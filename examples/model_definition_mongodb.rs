        ...
        Ok(created_id) => {
            ...
            match ModelManager::<User>::find_by_id(&created_id).await {
                Ok(Some(retrieved_user)) => {
                    ...
                    // 4. 演示JSON字段查询（MongoDB天然支持JSON查询）
                    println!("\n4. 演示基于JSON字段的查询...");

                    // 查询包含特定技能的用户
                    let json_query_conditions = vec![
                        QueryCondition {
                            field: "profile.skills.name".to_string(),
...
async fn demonstrate_performance_test() -> QuickDbResult<()> {
    ...
    match ModelManager::<User>::count(vec![]).await {
        Ok(total_count) => {
            assert!(total_count > 0, "性能测试中的总用户数统计应该大于0，但返回了{}", total_count);
    ...
    match ModelManager::<User>::find(batch_conditions.clone(), None).await {
        Ok(users) => {
            assert!(users.len() > 0, "性能测试中的批量查询（用户名包含'perfuser_'）应该返回至少1个用户，但返回了0个用户");
...
async fn main() -> QuickDbResult<()> {
    println!("=== RatQuickDB 模型定义系统演示 (MongoDB) ===");
    ...
    // 清理旧集合
    println!("🧹 清理旧集合...");
    let _ = rat_quickdb::drop_table("mongodb_default", "users").await;
    let _ = rat_quickdb::drop_table("mongodb_default", "articles").await;
    let _ = rat_quickdb::drop_table("mongodb_default", "comments").await;
    println!("✅ 集合清理完成");
