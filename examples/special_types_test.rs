...
define_model! {
    ...
    struct SpecialFieldsTest {
        ...
        is_read: bool,
        tags: Vec<String>,                    // 数组类型（JSON存储）
        metadata: serde_json::Value,          // JSON类型
        priority: i32,
...
async fn main() -> QuickDbResult<()> {
    ...
    rat_quickdb::init();
    println!("=== SQLite 特殊字段类型验证示例 ===");

    // 创建数据库配置 - SQLite配置
    let config = DatabaseConfig {
        db_type: DatabaseType::SQLite,
        connection: ConnectionConfig::SQLite {
            path: "special_types_test.db".to_string(),
