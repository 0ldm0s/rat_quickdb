...
async fn demonstrate_complex_queries() -> Result<QueryStats, Box<dyn std::error::Error>> {
    ...
    // 测试英文字段的大小写不敏感查询
    let english_ci_conditions = vec![QueryConditionWithConfig {
        field: "name".to_string(),
        operator: QueryOperator::Eq,
    ...
    let english_ci_result =
        ModelManager::<Product>::find_with_config(english_ci_conditions, None).await?;
    println!(
    ...
    // ============================================================================
    // 8. UUID字段查询 - 重要：MongoDB/MySQL/SQLite 使用 uuid_field() 时必须使用字符串查询
    // ============================================================================
    println!("\n8. UUID字段查询（重要场景）...");
    println!("   📝 说明: 在 MongoDB/MySQL/SQLite 中，uuid_field() 存储为字符串");
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== RatQuickDB MongoDB 查询操作完整演示 ===");
