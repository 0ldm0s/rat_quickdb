//! PostgreSQL 特殊字段类型验证示例
//!
//! 验证PostgreSQL适配器对特殊字段类型的处理，包括：
...
// 定义特殊字段测试模型
define_model! {
    /// 特殊字段测试模型 - 用于验证PostgreSQL特殊字段类型的处理
    struct SpecialFieldsTest {
        ...
        is_read: bool,
        tags: Vec<String>,                    // PostgreSQL数组类型
        metadata: serde_json::Value,          // JSON/JSONB类型
        priority: i32,
...
async fn main() -> QuickDbResult<()> {
    ...
    rat_quickdb::init();
    println!("=== PostgreSQL 特殊字段类型验证示例 ===");

    // 创建数据库配置 - 从timezone_complex_query_demo_pgsql.rs复制
    ...
    // 清理旧的测试表（确保每次都是干净的状态）
    println!("清理旧的测试表...");
    match drop_table("default", "special_fields_test").await {
